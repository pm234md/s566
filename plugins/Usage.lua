...
هرگونه استفاده رایگان از این فایل بدون خرید از سایت حرام است و نویسنده این پلاگین راضی نیست!
هرگونه انتشار رایگان فایل در کانال تلگرام یا سایت خود حرام است
-----------------
با خرید قانونی این فایل ,هم نویسنده پلاگین را حمایت کنید و هم حکم شرعی فایل را رعایت کنید
...
if matches[1] == "reply" and is_momod(msg) then 
    ...
    -------------Calc Run-------------
if matches[1] == "calc" and mathces[2] then 
    if msg.to.type == "user" then 
       return 
       end
